<div style="width:100%">
    ...
    <div class="viewheader" style="width:100%;padding-top:15px;">
		<div style="float:left;<?php echo (isset($data['hCol']) and $data['hCol']==2)?'width:45%;':'';?>">
	        ...
	        ?>
				<div style="padding-left:10px;float:left;width:150px;"><?php echo Inflector::humanize($label); ?></div>
			...
			if (isset($val[$model]) and !empty($val[$model])) {
				...
				foreach ($val[$model] as $j=>$k){
					...
					foreach($value as $i=>$v){ 
						...
						if (is_array($v) and isset($v['align'])) echo 'text-align:'.$v['align'];
	...
	<div style="position:absolute;top:260px;"><?php echo $data['catatan'];?></div>
	</table>
	<?php endif;?>	
	<?php //pr($data['tandaTangan']); ?>
	<?php if (isset($data['tandaTangan'])) :?>
	<table width="100%" style="padding-top:65px;font-size:10px;">
		<tr>
		...
		<tr><td style="height:<?php echo (isset($data['tandaTangan']['height']) and !empty($data['tandaTangan']['height']))?$data['tandaTangan']['height']:'30px';?>">&nbsp;</td></tr>
