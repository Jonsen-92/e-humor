...
<div class="<?php echo $controllername;?> index">
	...
	<tr>
		...
		<?php foreach ($aOpt['field'] as $i=>$v) : ?>
			...
			<th style="<?php if (isset($v['align'])) echo 'text-align:'.$v['align'].';'; if (isset($v['width'])) echo 'width:'.$v['width'].';';?>"><?php echo $this->Paginator->sort($w,$sort);?></th>
		<?php endforeach; ?>
			...
			<?php foreach ($aOpt['url'] as $x=>$y) : ?>
				<?php if (is_array($y) and !in_array($x, array('add','view','edit','delete'))) : ?>
					...
					<?php if (isset($y['alert'])) : ?>
						<?php echo $this->Html->link(__($x, true), $option_url, $option_link, sprintf(__($y['alert']. ' # %s?', true),$sFid)); ?>
					<?php else : ?>
