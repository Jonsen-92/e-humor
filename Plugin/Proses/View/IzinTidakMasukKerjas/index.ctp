...
<div class="index" style="overflow-x:auto;">
	<br/>
	<h2><?php echo __('Izin Tidak Masuk Kerja');?></h2>
	<div class="col-md-12">
		<div class="row">
	<?php echo $this->General->searchfiltering(array
		('nip_pemohon'=>array ('label'=>'NIP'),
	     'nama_pemohon'=>array ('label'=>'Nama Pegawai')));?>
	...
	<tr class="altrow">
		...
		<th><?php echo $this->Paginator->sort('tanggal_mulai','Jam Mulai'); ?></th>
		<th><?php echo $this->Paginator->sort('tanggal_akhir','Jam Akhir'); ?></th>
	...
	<tr<?php echo $i%2==0?' class="altrow"':'';?>>
		...
		<td><?php  echo $v[$model]['tanggal_mulai'];?>&nbsp;</td>
		<td><?php  echo $v[$model]['tanggal_akhir'];?>&nbsp;</td>
		...
		<?php
			...
			if($v[$model]['apr_pemberi_izin']=='PROSES'){
				...
				echo $this->Form->postLink(__(''), array('action' => 'delete', $v[$model]['id']),array('class' => 'btn fa fa-trash','title'=>'hapus'), __('Apakah anda yakin menghapus Izin Keluar Kantor ini# %s?'));
			}
			if($v[$model]['apr_pemberi_izin']=='DISETUJUI'){
				echo $this->Html->link(__('', true), array('action' => 'report', $v[$model]['id']),array('class' => 'btn fa fa-file','target'=>'_blank','title'=>'Cetak Form Cuti'));
			}
			if($v[$model]['alasan_apr']){
				echo $this->Html->link(__('', true), array('action' => '#', ''), array('class' => 'btn fa fa-comment-o','data-toggle'=>'modal','data-target'=>'#modal'.$v[$model]['id'],'title'=>'Lihat Alasan Penolakan'));
			}
                        ...
                        <div class="modal-dialog">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h5><b>Alasan Penolakan Izin</b></h5>
                                ...
                                <?php echo $this->Form->create('PengajuanCuti', array('type'=>'file','url'=>'disapprove/'.$v[$model]['id']));?>
                                    <div class="modal-body">
                                        <div class="form-group">
                                            <?php
												$alasanApr = isset($v[$model]['alasan_apr']) ? $v[$model]['alasan_apr'] : '-'; 

                                                echo $this->Form->input('alasan_apr',array('label'=>array('text' => 'Alasan Penolakan Izin', 'class' => 'col-md-3 control-label'),'type'=>'textarea','value'=>$alasanApr,'class'=>'form-control','between'=>'<div class="col-sm-12">','after'=>'</div>'));
                                            ?>
                                        </div>
                                    </div>
